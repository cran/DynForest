  ...
  for (p in 1:(length(unique(Y_boot$id))/2-1)){
    ...
    for (i in 1:length(feuilles_courantes)){
      ...
      if (length(unique(Y_boot$id[w]))>1 & imp_nodes[[feuilles_courantes[i]]] >0){
        ...
        if (N_courant >= nodesize*2){

          # Try best split on mtry factor predictors
          if (is.element("Factor",split.spaces)==TRUE){

            feuille_split_Factor <- var_split_scalar(X = Factor_courant, Y = Y_courant,
                                                     nodesize = nodesize)
          ...
          if (is.element("Curve",split.spaces)==TRUE){
            ...
            feuille_split_Curve <- var_split_scalar(X = Curve_courant, Y = Y_courant,
          ...
          if (is.element("Scalar",split.spaces)==TRUE){
            ...
            feuille_split_Scalar <- var_split_scalar(X = Scalar_courant, Y = Y_courant,
                                                     nsplit_option = nsplit_option,
                                                     nodesize = nodesize)
        ...
        if (decoupe>0){
          ...
          if (LN>=nodesize & RN>=nodesize){
            ...
            hist_imp_nodes <- rbind(hist_imp_nodes, c(2*feuilles_courantes[i],imp_nodes[[2*feuilles_courantes[i]]], length(which(feuille_split$split==1))))
            hist_imp_nodes <- rbind(hist_imp_nodes, c(2*feuilles_courantes[i]+1,imp_nodes[[2*feuilles_courantes[i]+1]], length(which(feuille_split$split==2))))
